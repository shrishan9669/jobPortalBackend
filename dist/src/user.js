...
const resend = new Resend("re_jnUSoZut_Q8526awAbZgTmbG2TCKRc4Nm");
...
userRouter.get('/getDashboardParams', requireAuthEmployer, async (req, res) => {
    const { id } = req.user;
    if (!id) {
        return res.json({
            ok: false,
...
////----Uploading resume...
userRouter.post('/upload_resume', requireAuth, upload.single('file'), async (req, res) => {
    const { id } = req.user;
    if (!id) {
        return res.json({
            ok: false,
...
userRouter.delete('/deleteResumeFromCloud', requireAuth, async (req, res) => {
    const { id } = req.user;
    if (!id) {
        return res.json({
            ok: false,
...
//'''''''''''''''IMAGE_UPLOAD-----------
userRouter.post('/uploadImage', requireAuth, upload.single('profile'), async (req, res) => {
    const { id } = req.user;
    if (!id) {
        return res.json({
            ok: false,
...
userRouter.delete('/removeProfilePic', requireAuth, async (req, res) => {
    const { id } = req.user;
    if (!id) {
        return res.json({
            ok: false,
