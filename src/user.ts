...
userRouter.delete('/deleteEmployee',async(req:any,res:any)=>{
...
userRouter.post('/userUpdate',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.post('/google-login',async(req:any,res:any)=>{
  try{
    ...
    const {token} = req.body ; // token is Google id_token from frontend
      if (!token) return res.status(400).json({ok:false, msg: "id_token missing" });
      ...
      if(!payload) return req.status(401).json({ok:false,msg:"Invalid google token"});

      const email = payload.email
      


      if (!email) return res.status(400).json({ ok:false,msg: "Email not available in token" });
...
userRouter.get('/getEducation',requireAuth,async(req:any,res:any)=>{
   const {id} = req.user;
   if(!id){
    return res.json({
      msg:"Id is required!!"
...
userRouter.post('/postProjects',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      msg:"ID required!!",
...
userRouter.delete('/deleteProject',requireAuth,async(req:any,res:any)=>{
...
userRouter.get('/getProjects',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
...
userRouter.post('/VerifyEmailLink',requireAuth,async(req:any,res:any)=>{
  const {email} = req.user;
  if(!email){
    return res.json({
...
//..........................................................................................
// Emploer Side.....................
userRouter.post('/sendMailOtp',async(req:any,res:any)=>{
...
userRouter.post('/employPreDetails',async(req:any,res:any)=>{
...
userRouter.get('/emailVerify',async(req:any,res:any)=>{
  const {email} = req.query;
  if(!email){
    return req.json({
      ok:false,
...
userRouter.post('/employPostDetails',async(req:any,res:any)=>{
...
userRouter.get("/checkVerify",async(req:any,res:any)=>{
...
userRouter.post('/employerLogin',async(req:any,res:any)=>{
...
userRouter.get('/ResendOTP',async(req:any,res:any)=>{
...
// Job Posting
// Required Auth
userRouter.post('/postingJob',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.get('/getAllJobs',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.delete('/deleteAJob',requireAuthEmployer,async(req:any,res:any)=>{
...
userRouter.post("/updateJob",requireAuthEmployer,async(req:any,res:any)=>{
...
userRouter.get('/getEmployer',requireAuthEmployer,async(req:any,res:any)=>{
    const {id} = req.user;
...
userRouter.post('/updateEmployer',requireAuthEmployer,async(req:any,res:any)=>{
...
userRouter.get('/getDashboardParams',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
  ...
  try{
    ...
    // Finding Total Applicants for this Employer
     const Jobs = await prisma.job.findMany({
      where:{
        employerId:Number(id)
      },
     ...
     for(let i=0;i<Jobs.length;i++){
        TotalAppli+=(Jobs[i]?._count.Applicants || 0)
     }
    ...
    const NewApplications = await prisma.applicant.count({
      where:{
         job:{
          employerId:Number(id)
         },
    ...
    let Interviews = await prisma.interview.count({
       where:{
...
//Recommended Jobs
userRouter.get('/recommendedJobs',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.get('/getJobsForUser',requireAuth,async(req:any,res:any)=>{
...
userRouter.get('/getSpecificJob/:jobid',requireAuth,async(req:any,res:any)=>{
   const {id} = req.user;
   if(!id){
    return res.json({
      ok:false,
...
userRouter.post('/savingAJob',requireAuth,async(req:any,res:any)=>{
...
userRouter.get('/getSavedJobs',requireAuth,async(req:any,res:any)=>{
   const {id} = req.user;
   if(!id){
    return res.json({
      ok:false,
...
userRouter.delete('/unsaveJob',requireAuth,async(req:any,res:any)=>{
    const {id} = req.user;
    if(!id){
      return res.json({
        ok:false,
...
userRouter.post('/createApplicant',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
  ...
  try{
    ...
    const New_Applicant = await prisma.applicant.create({
      data:{
        userId:Number(id),
        jobId:Number(jobId),
        resumeUrl:ApplyKarneWala.resumeUrl,
...
userRouter.get('/getAppliedJob',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
function calculateSkillMatch(jobSkills: string[], userSkills: string[]) {
...
function calculateExperienceScore(
  ...
  if (applicantExp >= requiredMin && applicantExp <= requiredMax) return 100;
  if (applicantExp >= requiredMin - 1) return 60;
  return 20;
}
function calculateLocationScore(jobLocation: string, userLocation?: string[]) {
  if (!userLocation) return 50;
  if (jobLocation === "Remote") return 100;
  if (userLocation.includes(jobLocation)) return 100;
  return 40;
}
function calculateProfileScore(user: any) {
  let score = 0;
  if (user.profileSummary) score += 40;
  if (user.resume) score += 40;
  if (user.skills?.length >= 5) score += 20;
  return score;
}

function calculateFitScore(job: any, applicant: any) {
  const skill = calculateSkillMatch(job.skills, applicant.skillsSnapshot);
  const exp = calculateExperienceScore(
    Number(job.experience.split('-')[0]),
    Number(job.experience.split('-')[1]),
    Number(applicant.experienceSnapshot.split('-')[1])
  );
...
userRouter.get('/getApplicants',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
function getNotificationMessage(action: string, jobTitle: string, company: string) {
  switch (action) {
    case "Shortlisted":
...
userRouter.post('/updateStatusByEmployer',requireAuthEmployer,async(req:any,res:any)=>{
...
userRouter.get('/getNotificationsUser',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.put('/markAllread',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.get('/getNotiCount',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.get('/getShortlistedCandidates',requireAuthEmployer,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
...
userRouter.post('/setInterview',requireAuthEmployer,async(req:any,res:any)=>{
    const {id} = req.user;
    if(!id){
      return res.json({
        ok:false,
    ...
    try{
       ...
       const Applicant = await prisma.applicant.findUnique({
          ...
          if (!Applicant) {
            return res.json({ ok:false, msg:"Applicant not found" })
          }
...
////----Uploading resume...
userRouter.post('/upload_resume',requireAuth,upload.single('file'),async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
   ...
   if(!req.file) {
      ...
      if (!["application/pdf"].includes(req.file.mimetype)) {
         return res.status(400).json({ ok:false, msg:"Only PDF files are allowed" });
      }
    ...
    try{
       const stream = cloudinary.uploader.upload_stream(
        ...
        async (error:any, result:any) => {
            ...
            // ✅ Abhi DB me save karo
          try {
            await prisma.user.update({
              where: { id:Number(id) },
              data: { resumeUrl: result.secure_url }, // ya jo bhi column hai
            });
          } catch (dbErr) {
            console.error("DB error:", dbErr);
            return res.status(500).json({ ok: false, msg: "Failed to save resume in DB" });
          }
...
userRouter.delete('/deleteResumeFromCloud',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
  ...
  const public_id = `resumes/${id}/resume.pdf`

  try{

     // Optional: check DB (safe practice)
    const user = await prisma.user.findUnique({
      where: { id: Number(id) },
    ...
    if (!user?.resumeUrl) {
      return res.status(404).json({
        ok: false,
        msg: "No resume to delete"
      });
    }


    // emptying user resumeurl field
     await prisma.user.update({
      where:{
        id:Number(id)
      },
...
userRouter.post('/uploadImage',requireAuth,upload.single('profile'),async(req:any,res:any)=>{
    const {id} = req.user;
...
userRouter.delete('/removeProfilePic',requireAuth,async(req:any,res:any)=>{
  const {id} = req.user;
  if(!id){
    return res.json({
      ok:false,
  ...
  try{
    ...
    if(!User?.profilePic){
      return res.json({
        ok:false,
        msg:"No profile pic to remove!!"
      })
    }

    const profile_url = User?.profilePic
    const file_extension = profile_url?.split('/').pop()?.split('.')[1];
    const public_id = `images/${id}/${file_extension}`
    ...
    // destroying specific image id folder
